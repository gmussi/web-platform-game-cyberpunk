...
export class MapEditorScene extends Phaser.Scene {
    ...
    createEditorUI() {
        ...
        // Instructions
        this.add
            .text(50, 60, "Select a tool first, then click to place objects | Right-click to remove | Drag to paint tiles", {
            fontSize: "12px",
            fill: "#ffffff",
            fontStyle: "bold",
            stroke: "#000000",
    ...
    createToolSelection() {
        ...
        tools.forEach((tool, index) => {
            const button = this.add
                .text(50 + (index % 4) * 120, 100 + Math.floor(index / 4) * 30, tool.name, {
                fontSize: "12px",
    ...
    closeSpritePicker() {
        ...
        const closeButton = this.children.list.find((child) => child.text === "Close" && child.depth === 1001);
        if (closeButton) {
            closeButton.destroy();
        }
        // Destroy title
        const title = this.children.list.find((child) => child.text === "Select Sprite" && child.depth === 1001);
        if (title) {
            title.destroy();
        }
    }
        ...
        this.input.on("pointerdown", (pointer) => {
            ...
            if (pointer.rightButtonDown()) {
                ...
                if (this.selectedTool &&
                    ["ground", "platform", "wall", "solid", "erase"].includes(this.selectedTool)) {
        ...
        if (this.mapData.tiles && Array.isArray(this.mapData.tiles)) {
            ...
            for (let y = 0; y < Math.min(this.mapData.tiles.length, this.tilemapSystem.mapHeight); y++) {
                if (this.mapData.tiles[y] && Array.isArray(this.mapData.tiles[y])) {
                    for (let x = 0; x <
                        Math.min(this.mapData.tiles[y].length, this.tilemapSystem.mapWidth); x++) {
...
//# sourceMappingURL=MapEditorScene.js.map