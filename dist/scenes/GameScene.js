...
export class GameScene extends Phaser.Scene {
    ...
    addAtmosphericElements() {
        ...
        for (let i = 0; i < 10; i++) {
            const particle = this.add.circle(Math.random() * 4100, Math.random() * 600, 1 + Math.random() * 2, 0x00ffff, 0.5);
    ...
    createUI() {
        ...
        this.healthText = this.add
            .text(this.healthBarX, 15, `${gameData.maxHealth}/${gameData.maxHealth}`, {
            fontSize: "14px",
            fill: "#fff",
            fontStyle: "bold",
        })
    ...
    update() {
        ...
        if (this.player && this.portalSprite) {
            const distance = Phaser.Math.Distance.Between(this.player.x, this.player.y, this.portalSprite.x, this.portalSprite.y);
            if (distance < 60) {
